...
export async function POST(req) {
    try {
        ...
        const { rows } = await query(
            `
      select id, dni, email, password_hash, app_role, member_type, member_number, is_active
      from users
      where dni = $1
      limit 1
      `,
        ...
        const res = NextResponse.json({
            ok: true,
            user: { id: user.id, dni: user.dni, email: user.email, app_role: user.app_role, member_type: user.member_type },
        });
